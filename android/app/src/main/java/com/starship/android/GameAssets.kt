 ...
 *
 *     Android/data/com.starship.android/files/
 *       baserom.us.rev1.z64  the user's ROM, copied in by the launcher
 *       config.yml, assets/  Torch's extraction recipes, unpacked from the APK
 *       starship.o2r         engine assets, built on CI and shipped in the APK
 *       sf64.o2r             generated on-device from the ROM
...
object GameAssets {
    ...
    // the contents, so the name says nothing about which revision it holds.
    const val ROM_NAME = "baserom.us.rev1.z64"

    /**
     * The ROMs that yield sf64.o2r, by SHA-1.
     *
     * Only the US revisions are here on purpose. config.yml routes JP, EU and CN
     * to `mods/sf64jp.o2r` and friends — those are language overlays layered on
     * top of the US archive, not a base game — so extracting one of them would
     * finish without ever producing the archive the engine boots from.
     */
    val SUPPORTED_ROMS = mapOf(
        "09f0d105f476b00efa5303a3ebc42e60a7753b7a" to "Star Fox 64 (U) (V1.1)",
        "f7475fb11e7e6830f82883412638e8390791ab87" to "Star Fox 64 (U) (V1.1) (Uncompressed)",
        "d8b1088520f7c5f81433292a9258c1184afa1457" to "Star Fox 64 (U) (V1.0)",
        "63b69f0ef36306257481afc250f9bc304c7162b2" to "Star Fox 64 (U) (V1.0) (Uncompressed)"
    )

    private const val TAG = "GameAssets"
    private const val GAME_ARCHIVE = "sf64.o2r"
     ...
     * recipes and a new starship.o2r without the user clearing data. Returns an
    ...
    /**
     * Runs Torch over [romFile] to produce sf64.o2r. Blocking and slow — expect
    ...
    init {
        // libStarship.so carries both the game and the Torch extractor.
