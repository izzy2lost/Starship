 ...
 * turn it into sf64.o2r before handing off to the game.
 *
 * No game data is shipped in the APK — the ROM stays on the device and the
 * extraction runs here, on demand.
 */
class LauncherActivity : ComponentActivity() {
     ...
     *
     * The unpacking moves megabytes out of the APK, so it cannot run on the main
     * thread.
     */
    private fun prepare() {
        setBusy(true, getString(R.string.launcher_preparing))

        thread(name = "sf64-prepare") {
        ...
        thread(name = "sf64-rom-import") {
            val result = runCatching {
                ...
                if (sha1 !in GameAssets.SUPPORTED_ROMS) {
                    partial.delete()
                    error(
                        "That is not a supported Star Fox 64 ROM.\n\n" +
                            "Supported:\n" +
    ...
    private fun startGame() {
        ...
        // still holding the ROM and every decoded asset, and otherwise it is
        // still the game library mapped in just to reach that one JNI call.
        // startActivity() has already reached the activity manager by this
        // point, so the game starts either way.
        Runtime.getRuntime().exit(0)
    }

    /** Spelled out from the real path so debug builds don't point at the release one. */
    private val romInstructions: String
        get() = "Select your own Star Fox 64 (US) .z64 ROM. It is verified, kept in this app's private " +
            "storage and converted into the game's asset archive on this device — no game data ships " +
            "with the app.\n\n" +
